...
import image1 from '../assets/notification/image-1.svg'
...
function Notification({className, title}) {
  return (
    <div className={`${className || ""} flex items-center p-4 pr-6 bg-n-9/40 backdrop-blur border border-n-1/10 rounded-2xl gap-5`}>
        <img src={image1} width={62} height={62} alt='img' className='rounded-xl'/>
                ...
                {/* <ul className='flex -m-0.5 '>
                    {notificationImages.map((icon, index) => (
                        <li key={index} className='flex w-6 h-6 border-2 border-n-12 rounded-full overflow-hidden'>
                            <img src={icon} className='w-full' width={20} height={20} alt='item'/>
                        </li>
                    ))}
                </ul> */}
                {/* <p className="body-2 ">Biel</p> */}
